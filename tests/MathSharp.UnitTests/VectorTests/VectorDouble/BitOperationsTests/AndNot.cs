...
namespace MathSharp.UnitTests.VectorTests.VectorDouble.BitOperationsTests
{
    public class AndNotTests
    {
        ...
        public static IEnumerable<object[]> Data
        {
            get
            {
                static double AndNotF(double a, double b)
                {
                    ulong andNot = ~Unsafe.As<double, ulong>(ref a) & Unsafe.As<double, ulong>(ref b);
                    return Unsafe.As<ulong, double>(ref andNot);
                }
                ...
                return new[]
                {
                    ...
                    new object[]
                    {
                        ...
                        new Vector4d(AndNotF(0d, double.NaN), AndNotF(double.MinValue, -0.00000000023434d), AndNotF(double.PositiveInfinity, double.NegativeInfinity), AndNotF(1414123d, 0d))
                    }
                };
            }
        }
...
#pragma warning enable xUnit1019
        public void AndNot_Theory(Vector256<double> left, Vector256<double> right, Vector4d expected)
        {
            Vector256<double> vector = Vector.AndNot(left, right);
