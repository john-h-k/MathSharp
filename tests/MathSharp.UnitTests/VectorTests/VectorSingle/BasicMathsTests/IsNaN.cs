...
namespace MathSharp.UnitTests.VectorTests.VectorSingle.BasicMathsTests
{
    public class IsNaN
    {
        ...
        public static void IsNaN_Theory(Vector128<float> values, bool[] expected)
        {
            var isNaN = Vector.IsNaN(values);

            Assert.True(TestHelpers.AreAllEqual(expected, isNaN.AsInt32()));
        }
    }
}